...
- **Search + browse pipeline**: generate diverse queries, run GPT-5.1 built-in web search, fetch pages, and summarize sources.
...
1. Install dependencies and make the package importable:

```bash
pip install -r requirements.txt
# Option A (recommended): editable install so "python examples/run_research.py" finds deep_research
python -m pip install -e .
# Option B: run without installing by adding the repo src to PYTHONPATH for the current shell
export PYTHONPATH="$(pwd)/src:$PYTHONPATH"  # PowerShell: $env:PYTHONPATH="$(Get-Location)/src" + ";$env:PYTHONPATH"
```
...
search = GPTSearchProvider(model=config.model)
...
- `deep_research.search`: GPT-5.1 web search provider and a static provider for tests.
...
- **GitHub 显示 “This branch has conflicts that must be resolved” 无法合并 PR**：请按照 [docs/merge_conflict_resolution.md](docs/merge_conflict_resolution.md) 中的命令行分步指南，将最新的目标分支（通常是 `main`）合并到当前分支，解决冲突后重新提交并推送。
